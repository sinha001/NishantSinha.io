...
import { ArrowLeft, Calendar, Clock, ExternalLink, User, Search, Filter, X, PenTool, BookOpen } from "lucide-react"
import { Link } from "react-router-dom"
import { motion } from "framer-motion"
import { sampleBlogPosts } from "@/data/blogs"
  ...
  const fetchMediumPosts = async () => {
    try {
      ...
      const response = await fetch(
        "https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@nishantsinha_42481",
      )
      ...
      const formattedPosts: BlogPost[] = data.items.map((item: any) => ({
        ...
        author: item.author || "Nishant Sinha",
        thumbnail: item.thumbnail || extractImageFromContent(item.content),
      }))

      setPosts(formattedPosts)
    } catch (err) {
      setError("No published articles found yet. Check back soon for technical insights!")
      console.log("Medium RSS fetch info:", err)

      // Show sample posts for demonstration of future content
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50">
      ...
      >
        <Link to="/" className="flex items-center gap-2 text-slate-800 hover:text-blue-600 transition-colors">
          ...
          <span className="text-2xl font-bold">Nishant Sinha</span>
        </Link>
      ...
      <section className="py-12 px-6 max-w-7xl mx-auto">
        {error && (
          ...
          >
            ...
            <h3 className="text-xl font-semibold text-blue-800 mb-2">Blog Coming Soon!</h3>
            <p className="text-blue-700 mb-4">{error}</p>
            <p className="text-blue-600 text-sm">
              Below is a preview of the technical content I'll be sharing. Follow me on Medium to get notified when I
              publish!
            </p>
          </motion.div>
        )}
                ...
                <Card className="bg-white shadow-sm border-slate-200 hover:shadow-lg transition-all duration-300 hover:-translate-y-1 h-full">
                  <CardContent className="p-0">
                    ...
                    <div className="p-6">
                      ...
                      {error ? (
                        <div className="text-slate-500 italic">Coming Soon</div>
                      ) : (
                        <Link
                          to={post.link}
                          target="_blank"
          ...
          <Card className="bg-white shadow-lg border-slate-200 max-w-2xl mx-auto">
            <CardContent className="p-8">
              <h3 className="text-2xl font-semibold text-slate-800 mb-4">
                {error ? "Stay Updated!" : "Want to read more?"}
              </h3>
              <p className="text-slate-600 mb-6">
                {error
                  ? "Follow me on Medium to get notified when I publish my first technical article. I'll be sharing insights on full-stack development, automation, and modern web technologies."
                  : "Follow me on Medium for the latest articles on web development, automation, and technology insights."}
              </p>
              <Button asChild size="lg" className="bg-green-600 hover:bg-green-700 text-white">
                <Link to="https://medium.com/@nishantsinha_42481" target="_blank" rel="noopener noreferrer">
      ...
      {/* Footer */}
      <footer className="py-8 px-6 bg-slate-800 text-white mt-20">
        <div className="max-w-7xl mx-auto text-center">
          <p className="text-slate-300">© 2025 Nishant Sinha. Built with React and Tailwind CSS.</p>
        </div>
      </footer>
    </div>
  )
